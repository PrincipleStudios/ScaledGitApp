...
public sealed class PowerShellCommandInvoker(IOptions<GitOptions> options, PowerShellFactory psFactory, ILogger<PowerShellCommandInvoker> logger)
	: CommandInvoker<IPowerShellCommandContext>(logger)
{
	protected override async Task<T> RunGenericCommand<T>(ICommand<T, IPowerShellCommandContext> command)
	{
		...
		var context = new PowerShellCommandContext(pwsh, logger);
